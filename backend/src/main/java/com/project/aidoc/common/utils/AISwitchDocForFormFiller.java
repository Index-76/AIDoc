 ...
 */
public class AISwitchDocForFormFiller {
    private static final Logger log = LoggerFactory.getLogger(AISwitchDocForFormFiller.class);
    ...
    public static String getTextFileContent(String readFileId, String userId, FileService fileService)
            throws Exception {
        String targetFileName = readFileId + "_text.txt";
        File textFile = findTextFile(readFileId, targetFileName, userId, fileService);
        if (textFile == null)
            throw new Exception("未找到文本文件：" + targetFileName);
        byte[] content = fileService.getFileContent(textFile.getId(), userId);
        if (content == null)
            throw new Exception("读取文本文件内容失败：" + textFile.getId());
    ...
    private static FileSummary generateFileSummary(File readFile, File textFile, String userId, FileService fileService)
            throws Exception {
    ...
    public static Map<Integer, List<String>> decideFileMapping(
        ...
        String modelName = (userConfig != null && userConfig.getDecisionModelName() != null)
                ? userConfig.getDecisionModelName()
            ...
            for (FileSummary s : fileSummaries) {
                ...
                if (!match && s.getKeywords() != null) {
                    for (String kw : s.getKeywords()) {
                        for (String h : headers) {
                            if (h != null && kw.equalsIgnoreCase(h)) {
                                match = true;
                                break;
                            }
                        }
    ...
    private static String extractJsonFromResponse(String response) {
        ...
        if (markdownCleaned.startsWith("{") && markdownCleaned.endsWith("}"))
